...
export default function CategoryListPage() {
  ...
  const toggleStatus = async (id: number) => {
    setTogglingId(id);
    const res = await fetch(`/api/admin/categories/${id}`, { method: "PATCH" });
    const json = await res.json();
    setTogglingId(null);
    if (res.ok) {
      setData((prev) =>
  ...
  return (
    <Box p="lg" style={{ background: "#f4f6f9", minHeight: "100vh" }}>
      ...
      >
        <Group justify="space-between">
          ...
          <Button
            ...
            radius="md"
            onClick={() => router.push("/adminpanel/category/add")}
          >
            Add Category
          </Button>
        </Group>
      ...
      <Paper radius="md" shadow="xs" p="lg" style={{ background: "#fff" }}>
        <Title order={5} c="blue" fw={700} mb="md">
          Category List
        </Title>
                    ...
                    <Table.Td>
                      <Group gap={6}>
                        <ActionIcon
                          ...
                          title="Edit"
                          onClick={() =>
                            router.push(`/adminpanel/category/edit/${row.id}`)
                          }
                        >
                          <IconEdit size={15} />
                        </ActionIcon>
